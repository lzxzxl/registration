...
%表示相邻边
edges = [];
for i = 1:size(VesData3D.node_positions,1)
    for j = find(adj_3d(i,:))
        if j > i
            edges(end+1,:) = [i j];
        end
...
% ax = gca;
% ax.Visible = 'off';
% exportgraphics(ax,'C:\Users\Lzx\Desktop\配准\paper_miccai\experiment_picture\NonRigid\RAW_CTO1.png');
...
% softassign 超参数
opts.softassign = struct();
opts.softassign.T_init = 500;
opts.softassign.T_final = 1;
opts.softassign.T_update = 0.93;
opts.softassign.nNormIters = 60;
opts.softassign.sigma2 = -1;    % auto
opts.softassign.slackAff = 1e-3;  % auto
opts.softassign.epsMass = 0.01;   % auto
opts.softassign.Kcand = 7;   % 候选邻居数（推荐 8~20）
opts.softassign.Kout  = 5;    % 输出 barycenter 用的 topK（推荐 3~5）
...
ax = gca;
ax.Visible = 'off';
% exportgraphics(ax,'C:\Users\Lzx\Desktop\配准\paper_miccai\experiment_picture\NonRigid\Deformable_CTO1.png');
