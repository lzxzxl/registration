...
% inxx=find(deg2d==1);
% hold on
% plot(VesData2D.Centerline(inx,1),512-VesData2D.Centerline(inx,2),'r*');
% plot(VesData2D.Centerline(inxx,1),512-VesData2D.Centerline(inxx,2),'bo');
%%画结构对应图
% figure;
% subplot(1,2,1);
% nodes = unique([pf_2d(:); Ves_tree_2d(:)]);
% [~,s] = ismember(pf_2d, nodes);
% [~,t] = ismember(Ves_tree_2d, nodes);
% 
% G = digraph(s, t, [], string(nodes));   % 节点名就是原始索引
% p = plot(G, 'Layout','layered', 'Direction','down', 'ArrowSize',4);
% p.NodeLabel = string(nodes);
% 
% subplot(1,2,2);
% nodes = unique([pf_3d(:); Ves_tree_3d(:)]);
% [~,s] = ismember(pf_3d, nodes);
% [~,t] = ismember(Ves_tree_3d, nodes);
% 
% G = digraph(s, t, [], string(nodes));   % 节点名就是原始索引
% p = plot(G, 'Layout','layered', 'Direction','down', 'ArrowSize',4);
% p.NodeLabel = string(nodes);
...
ax = gca;
ax.Visible = 'off';
% exportgraphics(ax,'C:\Users\Lzx\Desktop\配准\paper_miccai\experiment_picture\Rigid\Tree_left1_nontext.png');
% 



% %%画对应点连线
% indexx=Nodes(idx).FeaturePair(:,1);
% indexy=Nodes(idx).FeaturePair(:,2);
% 
% indexx(indexy==0)=[];
% indexy(indexy==0)=[];
% 
% xx=VesData3D_Proj(indexx,:);
% yy=VesData2D.Centerline(indexy,:);
% 
% X=[xx(:,1)';yy(:,1)'];
% Y=[512-xx(:,2)';512-yy(:,2)'];
...
ax = gca;
ax.Visible = 'off';
% exportgraphics(ax,'C:\Users\Lzx\Desktop\配准\paper_miccai\experiment_picture\NonRigid\CARNet_left1_ori_nontext.png');