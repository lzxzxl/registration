        ...
        if (cur_x == end_x && cur_y == end_y) {
         ...
         for (int dx = -1; dx <= 1; dx++) {
            for (int dy = -1; dy <= 1; dy++) {
                ...
                if (img[nidx]!=0 && dist[cur_idx] + 0.2*imgproj_DT[nidx] + img[nidx]< dist[nidx]) {  //0.2为了契合中心线改的
                    dist[nidx] = dist[cur_idx] + 0.2*imgproj_DT[nidx] + img[nidx];
